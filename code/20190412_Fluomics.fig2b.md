...
leGenes <- leGenes %>%
  ...
  filter(Gene.stable.ID %in% rownames(mashirinDF)) %>%
  filter(Gene.stable.ID %in% rownames(mouldDF)) %>%
  filter(!duplicated(Gene.stable.ID))
```

```
## Error in rownames(mashirinDF): object 'mashirinDF' not found
```
...
```
## Error in seq.default(from = -1 * min(abs(range(mat))), to = min(abs(range(mat))), : 'from' must be a finite number
```
